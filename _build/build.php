...
class loginSettingPackage
{
    ...
    /**
     * loginSettingPackage constructor.
     *
     * @param $core_path
     * @param array $config
     */
        ...
        foreach ($plugins as $name => $data) {
            ...
            if (!empty($data['events'])) {
                foreach ($data['events'] as $event_name => $event_data) {
                    /** @var modPlugiloginSetting $event */
                    $event = $this->modx->newObject('modPlugiloginSetting');
...
$install = new loginSettingPackage(MODX_CORE_PATH, $config);
